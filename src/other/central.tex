...
\begin{theorem}
    Let $X$ and $Y$ be two random variables, and let \setQ be a set of distributions over the pair $(X,Y)$.
    Assume that for every \inSet{Q}, we have
    \[
        Q(X, Y) = P(\XIY) Q(Y),
    \]
    where $P$ is a fixed conditional distribution.
    If $C$ is a central distribution of \setQ for $Y$, then $P(\XIY)C(Y)$ is a central
    distribution of \setQ for $(X,Y)$.
\end{theorem}
...
\begin{theorem}
    ...
    If \infSeq{\est} is a consistent sequence of estimators for \param, and there exists a sequence \infSeq{\lambda}
    such that the sequence $\infSeq[]{\lambda_i -\entropy{Q}{\est_i \mid \paramVal}}$ converges

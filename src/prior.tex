\newcommand{\XIe}{\rv[]{X} \mid \rval[]{e}}
Assume that for an environment we have decided on a probabilistic model \M, and without any particular prior
knowledge we have chosen a distribution $P$ from that model.
Because of our problem requirements, we are only interested in the probability of a random variable \rv[]{X}
conditioned on an observed evidence \rval[]{e}.
Then, we ask an expert, who is familiar with the environment, to evaluate our distribution $P$.
...
So to evaluate $P$, he would try to measure how bad using the wrong distribution $P(\XIe)$ is, while
the true distribution is $Q$.

Let us assume a real number can be assigned to that quantity and denote it as $\D[\XIe]$.
It seems reasonable, as $P$ gets closer to $Q$ this measure decreases, and increases when $P$ diverges from $Q$.
Therefore, $\D[]$ actually is a measure of the divergence of $P$ from $Q$, when $Q$ is the true distribution.
...
Interestingly, KL divergence is closely related to the concept of entropy:
...
The first property simply states that the entropy of a distribution is related to the divergence of the uniform
distribution.
...
We know that the divergence of $P(X,Y)$ from $Q(X,Y)$ is actually a measure of the generic loss of using $P(X,Y)$
when we believe $Q(X,Y)$ is the true distribution.
To calculate this loss, we can assume that $X$ happens before $Y$.
In that way, this loss would include the loss of using $P(X)$, and then if we observe $X = x$, which can happen with
probability $Q(x)$, it would also include the loss of using $P(Y \mid x)$.
This property tells us that the right way for combining these individual terms is the addition.

Back to our example, the expert provides D as a measure of how good our distribution $P$ is.
What if we find a distribution $C$ that every expert thinks is a good distribution?
In other words $\D[]$ is a small value for any $Q$.
This is very similar to the concept of bayesian convergence when the posterior converges to a distribution regardless
of the prior distribution.
Even if we can not find a dist that has a small divergence, still using the distribution that minimizes that amount

...
Therefore, he thinks, instead of $P$, we should use a different distribution $Q$.
...
\begin{definition}[Collective Divergence Measure]
    \newcommand{\YIe}{\rv{Y} \mid \rval{e}}
    Let \rv{x}, \rv{y} be two query variables, and \Eset be an evidence set in a probabilistic model \model.
    We define the \emph{collective divergence measure} of $P \in \set{P}$ for $\QvarIEset$ and $\rv{Y}|\Eset$ as
    follows:
    \begin{equation*}
        \measure{D}_{\QvarIEset, \rv{Y}|\Eset}[P] =
        \maxQ \maxE \left\{ \Ex{Q}{\Dcond} + \Ex{Q}{\Dcond[][][\YIe][\YIe]} \right\} \cdot
    \end{equation*}
\end{definition}

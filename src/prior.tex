...
Consequently, to evaluate $P$, he would try to measure how bad using the wrong distribution $P(\XIe[])$ is, while he
...
Therefore, $\D[]$ actually would be a measure of the divergence of $P$ from $Q$, when $Q$ is the true distribution.
...
Alternatively, by assuming similar properties, we can show that if we accept entropy as a measure of the uncertainty
of a distribution, we should also accept KL divergence as the right way for measuring divergence between distributions.
...
Even if we cannot find a distribution that has a small divergence, still using the distribution that
minimizes this quantity seems a reasonable choice.

...
\begin{theorem}
    Assume \pmodel is a generic parametric model, and $\Qset = \{\seqRv{}\}$ is a query set in \Mt.
    ...
    \end{equation}
    We define a functional $\functional{}$ of an unnormalized probability measure $\rho$ as
    follows:
    \begin{equation}
        \functional{\rho} = \left( \prod_i p(\tIw[i]) \rhoMass \right)^{\frac{1}{|\Qset|}},
        \label{eq:fixed_point_cx}
    \end{equation}
    where $p$ is the normalized probability measure obtained from $\rho$, and
    \[
        \rhoMass = \exp \left\{\sum_{\x[i]} \pi(\xIw[i]) \ln p(\wIx[i]) \right\}\cdot
    \]
    If \cPrior is the prior distribution corresponding to the central probability measure for \Qset in \Mt,
    then $\mu \cPrior$ is a fixed point for $\functional{}$, where
    \[
        \mu = \exp \left\{\frac{1}{|\Qset|}\int \DxCond[\central] \dt[\cPrior[]] \right\} \cdot
    \]
\end{theorem}

\begin{proof}
    First, we show that the set of distributions of each \X[i] is a convex set.
    Let $Q^*(\x[i]) = \convexCond{Q}{\x[i]}$, for some \inP{Q_1, Q_2}, $0 \leq \alpha \leq 1$ and
    arbitrary $i, \x[i]$.
    \Mt is a parametric model.
    Therefore:
    \begin{align*}
        Q^*(\x[i]) &= \alpha \marginal[q_1]{\x[i]} + (1 - \alpha) \marginal[q_2]{\x[i]}\\
        &= \int \pi(\xIt[i]) (\convexCond{q}{\paramValue}) d\paramValue\cdot
    \end{align*}
    ...
    \begin{align*}
        \DxCond[\central] &= \DxCondExpandFull[\central][][ ] \\
        &= \DxCondExpandFull[\central][{\paramFuncValue[i]}][ ] \\
        &= \sumix \pi(\xIw[i]) \ln \frac{\cMass{\wIx[i]}}{\cMass{\paramFuncValue[i]}}\\
        &= \sum_i \ln \frac{\rhoMass}{\cMass{\paramFuncValue[i]}}\cdot
    \end{align*}
    That implies
    \begin{equation}
        \label{eq:fixed_point_proof}
        \sum_i \ln \cMass{\paramFuncValue[i]} = \sum_i \ln \rhoMass - \lambda \cdot
    \end{equation}
    For $\paramFunc[i](\paramValue) = \paramFuncValue[i]$, we have $\cMass{\paramValue, \paramFuncValue[i]}=\cPrior$.
    Hence,
    \begin{equation*}
        \cMass{\paramFuncValue[i]} = \frac{\cPrior}{\cMass{\tIw[i]}}\cdot
    \end{equation*}
    We substitute this equation into Equation~\ref{eq:fixed_point_proof}:
    \begin{align*}
        e^{\frac{\lambda}{|\Qset|}} \cPrior &= \left( \prod_i \cMass{\tIw[i]} \rhoMass \right)^{
            \frac{1}{|\Qset|}},
    \end{align*}
    where $|\Qset|$ is the number of elements of \Qset. \qedhere
\end{proof}
...
\newcommand{\sure}[1][\set{s}]{\mathFix{c_{#1}}}
\newcommand{\sureF}{\ell_f}
...
\begin{definition} [Sure Thing, Sure Failure]
    The \emph{sure thing} is a function \inA{\sure} such that for every \inS we have $\sure(s) = c$.
    The \emph{sure failure} is a function \inA{\sureF} such that for every \inS we have $\sureF(s) > 0$
\end{definition}
...
\begin{definition} [Expectation]
    ...
    \begin{itemize}
        \item For a sure thing \sure, we have $\Ex{\set{s}}{\sure} = c$.
        \item For a sure failure $\sureF$, we have $\Ex{\set{s}}{\sureF} > 0$.
    \end{itemize}
\end{definition}

We do not need to explicitly define probability.

\begin{definition} [Probability]
    Let $\event \subseteq \set{s}$, and \inA{\act} be a restricted action on \event, such that
    \[
        \act(s) =
        \begin{cases}
            1       & s \in \event, \\
...
\begin{definition}
    The conditional operator of is a linear operator on \A{s} such that
\end{definition}
\begin{definition} [Conditional Expectation]
    is a linear functional where is a restricted action.
\end{definition}
...
\begin{proof}
    If $\F = \lambda \G$, clearly $\F[v] = 0$ if and only if $\G[v] = 0$.
...
\begin{definition}[Restricted Action]
    A \emph{restricted action} to $\event \subseteq \set{S}$, denoted by \act, is a member of \A{s} such that $\act(s) = 0
    $ for any $s \notin \event$.
\end{definition}

\begin{definition} [Conditional Expectation]
    Let \Es be an expectation for the sample space \set{s} and $\event \subseteq \set{s}$.
    \Es conditioned on \event, denoted by \EsIe, is a linear map on \set{s} such that
    for any \inA{\act[]}, if $\Es[\act] = 0$ then $\EsIe[{\act[]}] = 0$,
    where \inA{\act} is a restricted action to \event such that $\act(s) = \act[](s)$ for every $s \in \event$.
\end{definition}

\begin{theorem}
    Let \inA{\act[]} and $\event \subseteq \set{s}$, where \set{s} is a sample space.
    Suppose \inA{\act} is a restricted action to \event such that $\act(s) = \act[](s)$ for every $s \in \event$.
    Then we have
    \begin{equation*}
...
\begin{proof}
    ...
    We define \Ae to be the set of all restricted actions to \event.
    \Ae is a subspace of \A{s}.
    We can easily verify that the restrictions of \Es and \EsIe to \Ae have the same null space.
    That implies by Lemma~\ref{lm:scale} for any \inA{\act}
    \begin{equation*}
        \EsIe[\act] = \lambda \Es[\act] \cdot
    \end{equation*}
    Since for the sure thing \sure we have $\EsIe[\sure] = 1$ this implies
    \begin{equation*}
        \lambda = \frac{1}{\Es[{\sure[\cap \event]}]} = \frac{1}{P(\event)}
    \end{equation*}
\end{proof}

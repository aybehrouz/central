...
Our evaluation measure should not depend on the utility function, since that would imply that our belief about the
...
To obtain a measure that is not conditioned on a specific value of the evidence, we should calculate the expected value
of this KL divergence with respect to $Q$, which represents our expert's belief about the environment.

Now that we have a divergence measure to evaluate a distribution, what if we find a probability measure $C$ that
has a small divergence from any other distribution in our model?
That way, every expert would agree that our probability measure is a good choice.

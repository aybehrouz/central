...
\begin{proof}
    ...
    \[
        \kl[][\cDist_n][S_n] = \mu_n .
    \]
    By the definition of central distribution, that implies
    \begin{equation*}
...
\begin{proof}
    ...
    \]
    and provided that $C^*$ is a probability measure, by
    definition, it is a central distribution for \infSeq[]{(S_n, T_n)}.
\end{proof}
...
\]
we observe that for every \paramVal
\begin{equation*}
    \toInf  \left( \Dkl{\estLike}{C(\est_n)} - \lambda_n \right) = 0 \cdot
\end{equation*}
Under appropriate regularity assumptions ensuring that this convergence is uniform over \paramVal, the conditions of
Theorem~\ref{th:limiting-centrality} are satisfied.
Thus, the above asymptotic argument indicates that $\cPrior(\paramVal)$ acts as a central prior of \setQ for the
...
\begin{example}[Fused Multinomial Model]
    ...
    \begin{equation}
        R(S_n \mid T_n) = \frac{1}{n!} \prodm \frac{n_y!}{|f^{-1}(y)|^{n_y}} \cdot

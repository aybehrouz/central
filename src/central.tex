...
\newcommand{\myDiv}[1][P(X)]{\measure{D}{\mathrm{CD}}{\set{H}(X) \,\middle\|\, #1}}
...
\begin{definition}[Central Distribution]
    ...
    We call a distribution \ccDist a \emph{central distribution} of \setQQ if,
    for every probability distribution $P$, we have:
    \begin{equation*}
        \sup_{\set{H} \in \setQQ} \myDiv[\ccDist(X)]  \leq \sup_{\set{H} \in \setQQ} \myDiv,
    \end{equation*}
    where $\measure{D}{\mathrm{CD}}{}$ denotes the central divergence as defined in
    Definition~\ref{def:central-divergence}.
\end{definition}

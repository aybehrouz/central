...
Consider a binomial model with parameter $\theta$.
...
For a binomial model with success probability $\theta$, we have
\[
    \Ex{}{b_k(S_n) \mid \paramVal} = \paramVal^k,
\]
and
\[
...
When the approximation $\tilde P$ is close to an unnormalized marginal likelihood within the binomial family,
the bound in Theorem~\ref{th:central-approx} can be estimated by
\[
    \max_{\inSet[\paramVal]{S}} \engFunc[Q_{\paramVal}] - \min_{\inSet[\paramVal]{S}} \engFunc[Q_{\paramVal}],
\]
...
The corresponding predictive probability of success under \cDist is $5/14$.
...
For comparison, the absolute error of the predictive probability obtained under the uniform prior is shown as the
dotted line (Uni-Diff).

...
choice.
However, if the query and evidence variables, $\rv{x}$ and $\rv{e}$, depend on our specific random experiment, we
will violate the principle of objectivity.
To avoid this, one natural solution is to let the query variable \Qvar correspond to the joint of several variables
that can be used for calculating all probabilities that are relevant.

In our example, we can let $\Qvar = (\rv[k+1]{x}, \rv[k+2]{x}, \dots, \rv[n]{x})$, representing all dice rolls from $k$
to $n$.

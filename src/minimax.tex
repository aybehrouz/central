...
\begin{proof}
    ...
    \[
        \max_{R} f(\saddP, R) = \supD[\saddP].
    \]
    Thus, Equation~\ref{eq:th_proof_saddle_lq_dx} implies
    \[
        \supD[\saddP] \leq \supD[\cDist].
    \]
...
\begin{proof}
    ...
    the definition of a central distributing implies
    \[
        \supD[C'] = \lambda.
    \]
    Therefore, by non-negativity of the KL divergence, we have
    \[
        \kl[\cDist][C'] = 0. \qedhere
    \]

    By Theorem~\ref{th:central-support}, \cDist and $C'$ have the same support.
    Thus, they are identical distributions.
\end{proof}

\begin{corollary}
    \label{cor:C-maximizes-g}
    Under the assumptions of Theorem~\ref{th:lambda-centrality}, $C$ is a maximizer of the function $g$ defined in

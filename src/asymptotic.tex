...
\begin{proof}
    ...
    \begin{align*}
        \klx[{C_n}][C^*][S_n, T_n] &= \klx[{R(S_n \mid T_n)C_n}][R(S_n \mid T_n)C][T_n] \\
        &= \klx[{C_n}][C][T_n].
    \end{align*}
    Since $C$ is a central distribution for \infSeq{T}, by definition,
    \[
        \toInf \klx[{C_n}][C][T_n] = 0.
    \]
    It follows that
    \[
        \toInf \klx[{C_n}][C^*][S_n, T_n] = 0,
    \]
    and therefore $C^*$ is a central distribution for \infSeq[]{(S_n, T_n)}.
\end{proof}

Theorems~\ref{th:limiting-centrality} and~\ref{th:estimator-to-infseq} establish the conditions for asymptotic

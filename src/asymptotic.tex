...
\begin{proof}
    For this proof, we additionally assume that for each \(S_n\), there exists
    ...
    elements of \setQ.
    Therefore, $\kl[\cDist_n][\cDist][S_n]$ is a convex combination of
    \[
        Q_i(s_n) \ln \frac{\cDist_n(s_n)}{\cDist(s_n)}, \quad Q_i \in \setQ.
    \]
    By the previous result, we can conclude
    \[
        \toInf \kl[\cDist_n][\cDist][S_n] = 0.
    \]
    Therefore, by Definition~\ref{def:infseq-central}, \cDist is a central distribution of \setQ for the
    sequence \infSeq{S}.
\end{proof}

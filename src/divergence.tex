...
However, if we choose our query set based on our specific experimental setup, we will violate the
...
We may assume that our set contains $n$ pairs.
...
This implies that we could alternatively define our query set as a set containing a single pair $\Qset = \{ (\X,\E) \}$,
...
We usually simplify our notation and omit dummy events: $\Qset = \{\X\}$.
...
Back to our dice example, suppose we are interested in predicting the next dice outcome based on the number of previous
...
If we want our central probability measure to exhibit good convergence properties for  $P(\X[i] \mid \seqRv[Y]{i-1})$,
we need to explicitly include these conditional pairs in our query set, in the same way we did for $\X[i]|\seqRv{i-1}$.

After adding these pairs, the central probability measure for this new query set shows some interesting feature
selection capabilities.
In this probability measure, for smaller values of $i$, $P(\X[i] \mid \seqRv{i-1})$
tends to get somewhat combined with $P(\X[i] \mid \seqRv[Y]{i-1})$.
For example when $i = 2$, if the single observed dice roll is $01$, the posterior probabilities of
$00,10$ and $11$ are not equal, and $P(11 \mid 01)$ is slighty higher than $P(00 \mid 01)$ and $P(10 \mid 01)$.
That is to say this central probability measure detects that $01$ and $11$ are both odd numbers.

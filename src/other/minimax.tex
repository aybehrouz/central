...
\begin{proof}
    ...
    \]
    Since \cDist is a central distribution, we have \[\supD[\saddP] = \supD[\cDist],\] and $\saddP$ is also a central
...
\begin{theorem}
    \label{th:lambda-crossing}
    Assuming the conditions of Theorem~\ref{th:lambda-centrality} and using the same $\lambda$,
    if $P$ is a convex combination of elements in \setQ, then there exists some $Q_0 \in \setQ$ such that
    \begin{equation*}
...
\begin{proof}
    ...
    This implies that $g(P) > \lambda = g(C)$,
    where $C$ is the distribution specified in Theorem~\ref{th:lambda-centrality}.

    However, by the conditions of Theorem~\ref{th:lambda-centrality}, the distribution $C$ is a saddle point of $f$.
    This leads to a contradiction, since Lemma~\ref{lm:c-is-maximizer} asserts that $C$ must be a
    maximizer of $g$.
...
distribution $Q(\xIt)$.
Then, the weight function, $\saddR(i)$, will essentially become a prior distribution over \paramVal.

...
\begin{example}[Attribute Selection]
    ...
    The class label was defined as a deterministic function of a fixed subset of 3 out of 12 total attributes. The
    same relevant features and labeling function were used across all trials.

    For each experiment, we generated a training set of size $n$ and a test set of size $n/2$.
    ...
    function and one allowing for a conditional (non-deterministic) label distribution.
    As a baseline, we also trained a decision tree classifier.
    All methods were trained on the same training data and evaluated on the same test data.
    Each configuration was repeated for $k$ independent trials, and we reported the average accuracy (correct

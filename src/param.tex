...
\begin{theorem}
    ...
    \begin{equation*}
        \f = \fExpand,
    \end{equation*}
    where $\r$ is a probability distribution, and $\P$ is a vector that includes an
    entry $\Pentry \geq 0$ for every $i, \xe[i]$, such that $\sum_{\x[i]} \Pentry = 1$.

    $\f[ ]$ is a concave-convex function and has a saddle point: $(\sad{r}, \PP)$.
    Furthermore, the distribution $P(\assign[i]{x} \mid \assign[i]{e}) = \PPentry$ is a central
    distribution for \Qset.
    If $\saddle$ is a saddle point of $\f[ ]$, and there is a prior distribution $p$ such that
    $\PPentry = \int \pi(\xIet[i]) \dt$, then the
    distribution $P(\assign[i]{x} \mid \assign[i]{e}) = \PPentry$ is a central distribution for \Qset.
\end{theorem}
...
\begin{proof}
    ...
    \[
        \intProd[\rr]{\DxCond[\central]} \leq \maxParam \DxCond[\central] = \Dx[\central] \cdot
    \]
    On the other hand, $\RR$ satisfies Equation~\ref{eq:kkt_Rlambda}, and the definition of the divergence
    measure implies
    \[
        \Dx[\RR] = \maxParam \DxCond[\RR] = \lambda = \f[\rr][\RRvector] \cdot
    \]
    Thus,
    \[
...
\begin{proof}
    ...
    $\rr$ is a distribution and $\f[\rr][\central]$ is a weighted average of $\DxCond[\central]$ for
    different values of \paramValue.
    Therefore,
    \[
        \f[\rr][\central] \leq \Dx[\central]
    \]
    ...
    where $R$ is a probability measure in \Mt that is obtained by a positive prior $\rr(\paramValue) > 0$:
    \[
        \cdot
    \]
    ...
    For any fixed $\P$, $\f[\cdot]$ is a linear function.
    Note that if a function is concave (convex) on a set, it is also concave (convex) on any convex subset of that set.
    The domain of $\f[\cdot]$ is the set of all real functions $\r$ such that $\intProd[r]{} = 1$.
    We can easily verify that this set is a convex set, and therefore $\f[\cdot]$ is concave (and also convex) on the
    defined domain.

    On the other hand, for a fixed $\r$, if we let $Q(\xe[i]) =\marginal[r]{\xe[i]}$ we have
    \begin{equation}
        \f[r][\cdot] = c - \sumixe Q(\xe[i]) \ln \Pentry,
        \label{eq:max_for_P}
    \end{equation}
    ...
    Consequently, $\f[][\cdot]$ will be convex if the domain of valid $\P$-vectors is a compact convex set.
    For a valid $\P$ we must have $\interZeroOne{\Pentry}$ and $\sum_{\x[i]} \Pentry = 1$.
    We can easily verify that if $\P$ and $\P'$ satisfy the required conditions,
    $\alpha \P + (1 - \alpha) \P'$, where $\interZeroOne{\alpha}$ also satisfies these conditions.
    ...
    \begin{align*}
        ...
        &- \lambda \left( \intProd[\r]{} - 1 \right),
    \end{align*}
    and
    \begin{align*}
        \Lagr[\P][\lambda_{\e[i]}] = &\fExpand \\
        &+ \sum_{i} \sum_{\e[i]} \left(\lambda_{\e[i]}\left(\sum_{\x[i]} \Pentry - 1\right)\right) \cdot
    \end{align*}
    ...
    \[
        -\intProd[r]{ \frac{\pi(\xIet[i])}{\Pentry}} + \lambda(\e[i])
    \]
    ...
    \[
        \f[ ]\saddle  = \min_{\P} \f[\rr][],
    \]

    If $\P$ represents a valid distribution for each $\XIE[i]$ that can be obtained by a prior distribution, we
    have
    \[
        \Dx[\PP] = \f[ ]\saddle
    \]
    ...
    \]
    \begin{align*}
        \f[ ]\saddle &= \max_{\r} \f[][\PP] \\
        &= \min_{\P} \f[\rr][] \cdot
    \end{align*}
    This implies
    \[
        \Dx[\central] \geq \f[\rr][\central] \geq \f[\rr][\PP]
    \]

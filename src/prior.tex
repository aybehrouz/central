...
We should note that, in this context, we cannot objectively assert that in the environment there is some type of
underlying process for generating different values of \paramValue that does not assign zero probability to any
value of \paramValue.
...
We believe that this choice must depend solely on the environment.
As long as the environment is the same, we cannot change our choice.
For example if our utility function changes, or if the evidence that we get from the environment becomes more or
...
In particular, we are interested in the distribution of a random variable \rv[]{X}.
...
When evaluating $P$, he would try to measure the loss of using the incorrect distribution $P(\rv{X})$,
while believing the correct distribution is $Q(\rv{X})$.
...
Therefore, $\D[\rv{X}]$ effectively is a measure of the divergence of $P(\rv{X})$ from $Q(\rv{X})$, when $Q$ is
...
evaluate the optimal Bayesian strategy, in a generic way, non dependant to the utility function.

It can be shown that there always exists a deterministic optimal strategy.
That is, we can consider a strategy as a function of the evidence, rather than a conditional distribution:
\[
...
Hence, to evaluate the optimal strategy obtained by $P$, when we believe $Q$ is the correct probability measure, we can
calculate the expected value of the KL-divergence of $P(\XIe)$ from $Q(\XIe)$, with respect to $Q$.
Notice that this approach does not work for suboptimal strategies. Because for a suboptimal strategy,
$\envDist(\rv{E})$ plays a role and determines the importance of suboptimal actions.
...
$\epsilon$-convergence is closely related to the concept of Bayesian convergence.
An exchangeable sequence, i.e., an independent identically distributed sample set, \econverges when a sufficient
number of samples are observed.
...
Before making our prediction, we can roll the dice for several times, and the dice rolls are independent.
If we let \rv{e} be a vector representing the first 10 dice rolls and \rv{x} be the outcome of the next roll,
$\rv{x}|\rv{e}$ \econverges by the probability measure obtained from assuming a
uniform prior for the multinomial parameters when $\epsilon \geq 0.109$.
If \rv{e} represents the first 20 outcomes, then we can let $\epsilon \geq 0.073$.

Here, the chosen prior for the multinomial parameters affects the $\epsilon$-convergence.
In other words, different probability measures in our model have different convergence properties.
As we discussed earlier, the probability measure that gives the smallest possible $\epsilon$, is subjectively the safest
choice.
However, if during the process we choose a specific $\rv{x}|\rv{e}$ based on our experiment, we will violate the
principle of objectivity.

Thus, we need to define a set of query and evidence variables that reflect some assumptions about the
environment, and do not depend on our experimental setup.
One natural way for this is to let each \rv[i]{x} be the outcome of
the $i$-th dice roll, and define each \rv[i]{e} as a vector of the previous dice outcomes $(\seqRv{i-1})$.
Then, we will consider a sequence of conditionals of the form:
$\rv[k]{x}|\rv[k]{e},\ \rv[k+1]{x}|\rv[k+1]{e},\ \rv[k+2]{x}|\rv[k+2]{e},\ \dots,\ \rv[n]{x}|\rv[n]{e}$.
...
Definition~\ref{def:e-convergence} already determines the measure for a single query variable with different evidence variables.
...
Therefore, we believe that addition should be used.:
\[
    \sum_i \measure{D}
\]
which equals to
...
That way, every expert would agree our probability measure is a good choice.
...
When dealing with multiple query variables, the additivity property guides us in defining the appropriate divergence
measure:

\begin{definition}[Collective Divergence Measure]
    \newcommand{\XIeX}[1]{\rv{#1} \mid e_{\rv{#1}}}
    \newcommand{\EX}{\Eset_X}
    \newcommand{\EY}{\Eset_Y}
    Let \rv{x}, \rv{y} be two query variables, and $\EX$, $\EY$ be two evidence sets in a probabilistic
    model \model.
    We define the \emph{collective divergence measure} of $P \in \set{P}$ for $\rv{X}|\EX$ and $\rv{Y}|\EY$ as
    follows:
    \begin{equation*}
        \measure{D}_{X|\EX, Y|\EY}[P] =
        \max_{Q \in \set{P}, E_X \in \EX, E_Y \in \EY} \left\{ \Ex{Q}{\Dcond[][][\XIeX{x}][\XIeX{x}]} +
        \Ex{Q}{\Dcond[][][\XIeX{y}][\XIeX{y}]} \right\} \cdot
    \end{equation*}
\end{definition}

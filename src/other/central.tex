...
Within a Bayesian framework, however, inference ultimately requires a single probability measure.
...
for inference and also for evaluating a distribution based on our partial knowledge.

The concept of a central distribution is not intended to yield a unique object;
rather, all distributions satisfying the definition can be regarded as equally good representatives.
Therefore, while the subsequent definitions seem to depend on uniqueness, it can be made well-defined by
consistently choosing some central distribution, as all choices are interchangeable.
...
The above definition applies only when the hierarchy is a single set of distributions.
...
\begin{definition}
    ...
    Let $X$ be a random variable, and let \set{H^*} be a set whose elements are hierarchy of distributions over $X$.
    Let \cDist[\set{H}] be a central distribution of $\set{H} \in \set{H^*}$ for $X$.
...
This demonstrates that hierarchical organization carries informational content beyond the collection of leaf sets
alone.
...
structural form.
In particular, when the relevance of potential features is unknown, each candidate feature subset determines a distinct
family of distributions in which the target variable depends only on that feature set.
...
Candidate feature sets typically induce overlapping families of admissible distributions.
...
simpler models by assigning greater representation to distributions compatible with multiple structural hypotheses.

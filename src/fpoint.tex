...
\newcommand{\normConst}{\mu}
\newcommand{\solPrior}{c}
\newcommand{\solDist}{C}

\newcommandx{\leg}[2][1={\w[i]}, 2=\solPrior, usedefault]{\frac{\avgPost[{#1}][{#2}]}{#2(#1)}}
...
\begin{proof}
    ...
    First, note that for any arbitrary probability distribution $\someDist$, induced by a strictly positive prior $\somePrior$,
    we can write:
    \begin{align*}
    ...
    \begin{equation*}
        \fpMap{\fixedPoint}(\w) = \frontalPus[][\fixedPoint] ^{\frac{1}{k+1}} \fixedPoint(\w) = \fixedPoint(\w) \cdot
    \end{equation*}
    Therefore, $\fixedPoint$ is a fixed point for $\fpMap{}$.
    In addition, we can obtain Equation~\ref{eq:pus_th_conditional} for $\solPrior(\tIw)$ by dividing
    ...
    Substituting this into Equation~\ref{eq:pus_proof_main_eq} yields
    \[
        \DxCond[\solDist] = \ln \frontalPus \cdot
    \]
    Substituting Equation~\ref{eq:puss_proof_pus_is_mu} into the preceding equation yields
    \[
        \DxCond[\solDist] = (k + 1) \ln \normConst,
    \]
...
Additionally, it defines the mapping $\fpMap{}$ based on \w and its marginals \w[i], rather than \paramValue.
...
In models that the posterior of \X converges, we can calculate $\avgPost[][\cMass{}]$ by using any well behaved prior
instead of $\cMass{}$.

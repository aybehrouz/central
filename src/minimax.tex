...
Throughout this subsection, all random variables are assumed to be discrete with finite support.
...
\begin{proof}
    ...
    \]
    Since $\ln$ is upper semicontinuous on $[0,+\infty)$ and the summation is finite, $\kl$ is lower
    semicontinuous as a function of $P$.
    ...
    Hence, by the extreme value theorem for lower semicontinuous functions on compact sets, $\sFunc[P(X)][\setQ]$
    attains its minimum.
    Therefore there exists a probability distribution \cDist such that
    \[
        \sFunc[\cDist(X)][\setQ] = \inf_P \sFunc[P(X)][\setQ].
    \]
    By Definition~\ref{def:center-of-set}, $C$ is a central distribution of \setQ for $X$.
\end{proof}
...
\begin{proof}
    ...
    \]
    where $R$ is a probability distribution, and
    \[
...
\begin{proof}
    ...
    we obtain $\eta = 1$.
    Substituting this into Equation~\ref{eq:kkt_proof_R_is_pxIe} yields Equation Equation~\ref{eq:kkt_w_avg},
    which in turn implies Equation Equation~\ref{eq:kkt_proof_p_sums_one}.
...
\begin{theorem} [Centrality]
    ...
    Suppose that \cDist is a convex combination of elements of \setQ, that is,
    \[
        \cDist(x) = \sum_i Q_i(x) R(i),
    \]
    where $R$ is a probability distribution over the indices of $Q$.
    Assume that there exists a constant $\lambda$ such that
    \begin{equation*}
    ...
    Then \cDist is the unique central distribution of \setQ for $X$.
\end{theorem}
...
\begin{proof}
    ...
    Therefore, by Lemma~\ref{lm:saddle_conditions}, the point $\left( \cDist, R \right)$ is a saddle point,
    and Theorem~\ref{th:saddle_is_central} implies that \cDist is a central distribution of \setQ for $X$.
...
\begin{corollary}
    ...
    Assume the conditions of Theorem~\ref{th:centrality} and let $\lambda$ be the corresponding constant.
    Then, for any probability distribution $P$,
    \begin{equation*}
        \myDiv[P][Q][(X)] \leq \sFunc[P(X)][\setQ] - \lambda.
    \end{equation*}
\end{corollary}

\begin{proof}
    By Theorem~\ref{th:centrality}, \cDist is the unique central distribution of \setQ,
    and
    \[
        \myDiv[P][Q][] = \kl[\cDist].
    \]
...
\begin{proof}
    In the proof of Theorem~\ref{th:centrality}, we showed that $(\cDist, R)$ is a saddle point for $f$.
    By letting $C(X, I) = Q_i(X)R(I)$, Lemma~\ref{lm:saddle-is-maximizer} implies that \cDist is a maximizer
    of the function $g$.
\end{proof}
...
\begin{corollary}
    ...
    let $P$ be a convex combination of elements of \setQ, i.e.,
    \[
        P(x) = \sum_i Q_i(x) R(i),
    \]
    where $R$ is a probability distribution.
    Then there exists an index $k$ in the support of $R$ (that is, $R(k) > 0$) such that
    \begin{equation*}
        \lambda \geq \klx[Q_k].
    \end{equation*}
\end{corollary}
...
\begin{proof}
    ...
    Suppose that for every $k$ in the support of $R$, we have $\lambda < \kl[Q_k]$.
    That implies
    \[
        g(P) > \lambda = g(C),
    \]

...
\begin{proof}
    ...
    Hence, for all $i$, $x$ we have $\hat{R}(i)Q_i(x) \geq 0$, and $\hat{f}(P,\none)$  is
...
The following theorem provides the rationale behind the term ``central distribution.''
...
\begin{theorem}[Imperfection]
    \label{th:imperfection}
...
\begin{proof}
    \renewcommand{\c}{\cPrior(\paramVal{})}
    From Equation~\ref{eq:central-prior}, it follows that there exists a constant $\lambda$ such that
    \begin{equation*}
        \c = \exp \left\{-\lambda +  \sum_{x} \like(\xIt) \ln
                      \frac{\like(\xIt) \c}{\cDist(x)} \right\} \cdot
    \end{equation*}
    Since $\c$ is strictly positive, we have
    \begin{align*}
        \c &= \exp \left\{-\lambda +  \kl[Q_{\paramVal{}}][C] + \ln \c \right\} \\
        &= \c \exp \left\{-\lambda +  \kl[Q_{\paramVal{}}][C] \right\},
    \end{align*}
    which implies
    \[

...
\begin{proof}
    ...
    \[
        \Dparam[R] \geq 0\ \cdot
    \]
    Using this, for every distribution $P$ and every \vval{\theta}, we obtain
    \newcommand{\RP}{\sumOverixe \pi(\xeIt) \ln \frac{R(\xIe)}{P(\xIe)}}
    \[
        \RP \leq \DxPcond \cdot
    \]
    Since this inequality holds for any $\paramValue$, the maximum of the right-hand side w.r.t. $\paramValue$
    is also greater than or equal to the maximum of the left-hand side:
    \begin{equation}
...
\begin{theorem}
    ...
    Let \pmodel be a parametric model, and $\Xpairs$ be a query set in \Mt.
...
\begin{proof}
    ...
    \begin{align*}
        \Ex{R}{\sum_i \Dcond[R]} &= \DExparam[\paramValue^*] \\
        &= \tmax \DxPcond \cdot
    \end{align*}
    ...
    \]
    Notice that the left hand side is $\DxP$, and
    \[
...
Following similar steps we did for the query set \set{x}, we can show that \set{x'} also has a more compact counterpart,
containing a single pair $(\rvec{x}, \rvec{e'})$, where \rvec{x} is the vector $(\seqRv{n})$, and \rvec{e'} is
the vector
$(\seqRv[y']{n})$.

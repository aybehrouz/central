...
This implies that in a generic parametric model, we can obtain the central probability measure by:
\begin{equation*}
    \central = \arg\minP \maxParam \DxCond  \cdot
\end{equation*}

This is a challenging minimax problem that cannot be easily solved directly.
However, by using the minimax theorem we can convert it into a more conventional maximization problem which can be
...
A vector in $R^n$ shares many properties with a real function.
We sometimes assume that a real function is a real vector, and vice versa.
In the next theorem we will use this theorem, assuming $x$ is a real function.
...
\begin{theorem}
    ...
    a distribution $Q^* \in \set{P}$, such that for every $i, \xe[i]$ we have
    \begin{equation}
        Q^*(\xIe[i]) = \convexCond{Q}{\xIe[i]}\cdot
        \label{eq:convex_dist_set}
    \end{equation}
    Then, the central distribution for $\set{X}$ can be obtained by:
    \begin{align}
...
\begin{proof}
    ...
    To do so, we define a functional as follows:
    \begin{equation*}
    ...
    all values of $P(\assign[i]{x} \mid \assign[i]{e})$ for every $i, \xe[i]$, where $P \in \set{P}$.
    ...
    Moreover, from Equation~\ref{eq:convex_dist_set}, it follows that the domain of $P_{\XIE[i]}$ vectors is also a
    compact convex set.

    For a fixed $P_{\XIE[i]}$, $\f[\cdot]$ is a linear function, which is both concave and convex.
    ...
    Therefore, we can use Theorem~\ref{th:minimax}
    \begin{equation}
    ...
    \[
        \central = \arg\max_{R \in \set{P}} \int \DxCond[R] \dt[r] \cdot \qedhere
    \]
\end{proof}

Now, let's use this theorem for our dice example and rewrite Equation~\ref{eq:joint_form}:
\begin{align*}
    \central = \arg\max_{p} \int \sum_{\seqRval{n}} p(\seqRval{n}, \paramValue) \ln
...
\begin{proof}
    ...
    Let $Q^*(\x[i]) = \convexCond{Q}{\x[i]}$, for some $Q_1, Q_2 \in \set{P}$, $0 \leq \alpha \leq 1$ and
    arbitrary $i, \x[i]$.

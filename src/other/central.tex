...
\begin{example}
    Assume that $S_n$ is a sequence of $n$ independent trials of an experiment with $k$ outcomes.
...
\begin{example}
    ...
    $Y_i = f(X_i)$, where \( \mathcal{Y} = \{1, \dots, m\} \).

    Assume that given \( Y_i = y \), the conditional distribution \( P(X_i \mid Y_i = y) \) is uniform over the
    preimage \( f^{-1}(y) \).
    ...
    For a sequence \( S_n \), let \( n_y \) denote the number of times label \( y \in \mathcal{Y} \) appears in the sequence.
    Let \( |f^{-1}(y)| \) denote the number of outcomes in \( \mathcal{X} \) that are mapped to label \( y \) by \( f \).
    Then the probability of observing \( S_n \) under \( Q_\theta \in \mathcal{Q} \) is:
    \[
        Q_\theta(S_n) = \prod_{y=1}^m \left( \frac{1}{|f^{-1}(y)|} \cdot \theta_y \right)^{n_y}.
    \]

    This expression reflects that each \( Y_i \sim \text{Categorical}(\theta) \), and each \( X_i \) is drawn uniformly from the set \( f^{-1}(Y_i) \).
    The product captures the joint contribution of label frequencies and uniform uncertainty within each label.
\end{example}

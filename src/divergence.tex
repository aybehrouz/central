...
Let us assume that a real number can be used to measure this loss, and denote it as $\Div$.
...
Alternatively, by adopting a more minimal set of properties, one can prove that accepting Shannon’s entropy as a
measure of the uncertainty implies that KL divergence can be considered as an appropriate
method for measuring divergence between distributions.

\begin{theorem}
    Let $X$ be a discrete random variable.
    Suppose $\Div$ is some divergence measure that quantifies the divergence of $P(X)$ from $Q(X)$.
    ...
    \begin{description}
        ...
        \[
            \Dkl[]{Q(\varOmega)}{U(\varOmega)} = \entropy{U}{\varOmega} - \entropy{Q}{\varOmega},
        \]
        ...
        \[
            \Div[X,Y] = \Div + \sum_x \Div[Y|x]Q(x)\cdot
        \]
    \end{description}
    If $\measure{D}{}{\none}$ satisfies these properties, then it is the Kullback--Leibler divergence.
\end{theorem}
...
\begin{proof}
    ...
    \[\entropy{P}{\TIx} = \ln P(x),\ \entropy{P}{\TX} = 0 \cdot\]
...
In this theorem, the first property simply states that the entropy of a distribution should be linearly related to the
...
We know that the divergence of $P(X,Y)$ from $Q(X,Y)$ is a measure of the generic loss of using $P(X,Y)$
...
\begin{lemma}
    ...
    \begin{equation*}
        \klx \geq \klRatio \cdot
    \end{equation*}
\end{lemma}

\begin{proof}
    The Kullback–-Leibler divergence is nonnegative and is defined when $R$ is a probability distribution:
    \begin{equation*}
        \klx[][R] \geq 0 \cdot
    \end{equation*}
...
\begin{proof}
    The result follows from Lemma~\ref{lem:kl-ineq} after expanding the left-hand side of the inequality using the

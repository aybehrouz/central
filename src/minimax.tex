...
This characterization motivates the term ``central distribution.''
...
\begin{theorem} [Centrality]
    ...
    Then \setQ admits a unique central distribution for $X$.

    Moreover, \cDist is the \cd\ if and only if \cDist can be expressed as a convex combination of the elements
    of \setQ; that is,
    \[
...
\begin{proof}
    ...
    By Lemma~\ref{lm:saddle-is-central} the $\saddP$-component of any saddle point is a \cd.
    Let \cDist denote this central distribution.

    Every saddle point of $\objf[]$ satisfies the conditions of Lemma~\ref{lm:saddle_conditions}.
    ...
    To prove uniqueness, let $C'$ be an arbitrary probability distribution.
    We have
    ...
    Suppose that $C'$ is also a central distribution of \setQ for $X$.

...
\begin{proof}
    ...
    \end{align*}
    By the generalized chain rule of calculus, for the partial derivative of $\Lagr$ relative to $\epsilon$ we have
    \begin{align*}
        \parDeriv &= \int \parDeriv[][p(\paramValue)] \de{p(\paramValue)} d\paramValue +
        \sumixe \left( \parDeriv[][{P(\xe[i])}] \de{P(\xe[i])} + \parDeriv[][{P(\e[i])}] \de{P(\e[i])} \right) \\
        &= \int \left(-\lambda + \DxCondExpandFull  \right) \dt[\eta] \\
        &- \sumixe \left(\frac{1}{P(\xe[i])} \marginal{\xe[i]} \marginal[\eta]{\xe[i]} \right) \\
        &+ \sum_i \sum_{\e[i]} \left(\frac{1}{P(\e[i])} \marginal{\e[i]} \marginal[\eta]{\e[i]} \right) \cdot
    \end{align*}

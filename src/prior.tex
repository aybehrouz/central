...
We ask from an expert---a person familiar with the environment---to evaluate our distribution $P$.
...
Consequently, for evaluating $P$, he would try to measure how bad using the wrong distribution $P(\XIe[])$ is, while he
thinks the true distribution is $Q$.

Let's assume that a real number can be used to measure this concept, and denote it as $\D[{\XIe[]}]$.
...
Each \rv{x} represents a random variable whose distribution is important to us, and each \rv{e} corresponds to the
...
\begin{definition}[Central Distribution]
    In a model \model, we define the \emph{central distribution} for a query set \set{X} to be a distribution
    $C_\set{X} \in \set{P} $ which minimizes the divergence measure for \set{X}:
    \begin{equation}
...
we will need $P(\rv[4]{x} \mid \rv[1]{x},\rv[2]{x},\rv[3]{x})$, where \rv{x} denotes the outcome
of the $i$-th dice roll.

Thus, one natural way for defining a query set $\Xpairs$ for this problem is to let each \rv{x} be the outcome of
...
\end{align*}
We factor out $\pi(\seqRval{n} \mid \paramValue)$ from every term, then we can use the chain rule for conditional
probability:
\begin{align*}
    \DxPcond &=
...
\begin{align*}
    \Cx &= \arg\max_{P} \int \left(\KLcond{\seqRval{n}}{}\right) \dt \\
...
Now, imagine playing a game of chance involving our four-sided dice.
...
We just know that the machine is memory-less, and the result of each game is independent.

Through experimentation, we want to learn how that machine works, so that by observing the dice outcome we can predict
whether we will win the game.
We assume that, first, we roll the dice $n$ times, and record each outcome.
Next, we input a number of these dice outcomes into the machine and observe the result of each game.
As we will see, this assumption is important for easier derivation of the central distribution.

Let us define a query set for this problem.
We denote this set as $\Xpairs$, where each \rv{x} is the result of the $i$-th game, and the evidence \rv{e} is a
vector
\[
    (\seqRv{i-1}, \seqRv[y]{n}),
\]
...
This implies that an alternative way for defining our query set is to define it as a single
...
query set.
While each \rv{x} remains unchanged, we need to represent each \rv{e} as a vector
\[
    (\seqRv{i-1}, \seqRv[y']{n}),
\]
where each \rv{y'} represents the least significant digit of the dice outcome.
We denote this new query set with \set{x'}.
...
However, when there is not much data available, the simpler version can still produce some meaningful results,
...
versions of the problem based on the data that is observed?

...
By Theorem~\ref{th:central-approx}, we need to find a positive measure $\tilde P$ such that, for every \inSet{Q},
\[
    \engFunc \approx 0.
\]

In addition, the bound depends on how close the normalized distribution $P$ is to a convex combination of the
elements of \setQ.
...
However, in the case of sequences of random variables, central distributions are defined on the
...
One possible approach is to assume that $\ln \tilde P(X)$ can be represented as a linear combination of a collection
of basis functions $b_i$, in a manner similar to an exponential family:
\[

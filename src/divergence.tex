...
\[
    \D[\X] + \D[\Y].
\]
Separate evaluation of random variable distributions is essential for the divergence measure that we will define
shortly.
...
Furthermore, we cannot determine what value of $\epsilon$ would be small enough.
...
Unfortunately, that would make determining the appropriate value of $\epsilon$ even harder because that naturally
increases the value of $\epsilon$.

We could avoid all these hurdles by simpling choosing the probability measure that gives the smallest
possible $\epsilon$.
Subjectively, this probability measure is the ``safest'' choice in our model and it turns out that it performs well
even when the minimum achievable value of $\epsilon$ is not very small.
...
\begin{proposition}
    For any value of \paramValue, the conditional divergence is non-negative.
\end{proposition}

Now that we have established a divergence measure, we are ready to provide a formal definition for the probability
...
This implies that we could alternatively define our query set as a set containing a single pair $\Qset = \{ (\XE) \}$,
...
$00,10$ and $11$ are not equal, and $P(11 \mid 01) = 0.158$ is slightly higher than
$P(00 \mid 01) = P(10 \mid 01) = 0.133$.
It can be said that this central probability measure identifies that $01$ and $11$ are both odd numbers.

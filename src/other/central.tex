...
\newcommandx{\kl}[3][1=Q, 2=P, 3=X, usedefault]{ \measure{D}{\mathrm{KL}}{#1(#3) \,\middle\|\, #2(#3)} }
\newcommandx{\klExp}[3][1=Q, 2=P, 3=x]{\sum_{#3} #1(#3) \ln \frac{#1(#3)}{#2(#3)}}
\newcommand{\inSet}[1]{\mathFix{#1 \in \set{#1}}}

\newcommandx{\entropy}[2]{\measure{H}\ifthenelse{\isempty{#1}}{}{\left[#1_{#2}\right]}}
...
\begin{definition}[Central Distribution]
    ...
    We say a probability distribution $C$ is a central distribution of \setQ for $X$, if for any arbitrary distribution
    $P$, we have
    \[
...
\begin{definition}[Central Divergence]
    For any distribution $P$, we define the central divergence of $P$ from a set of distributions \setQ, with respect
    to a random variable $X$, as follows:
    \begin{equation*}

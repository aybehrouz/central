...
distribution $Q(\xIt)$.
Then, the weight function, $R(i)$, will essentially become a prior distribution over the parameter \param.
...
\begin{proof}
    ...
    \begin{equation}
        \label{eq:th5-proof}
        \sup_{Q \in \setQ} \Dkl{Q(X,T)}{R(\XIT) C(T)} \leq \sup_{Q \in \setQ} \Dkl{Q(X,T)}{R(\XIT) P(T)}.
    \end{equation}
    ...
    \[
        \sup_{Q \in \setQ} \Dkl{Q(X,T)}{R(\XIT) P(T)} \leq \sup_{Q \in \setQ} \Dkl{Q(X,T)}{P(X,T)}.
    \]
...
\begin{proof}
    ...
    by Theorem~\ref{th:lambda-centrality} it follows that \cDist is a central distribution of \setQ for $X$.
...
Although this theorem is proved directly from Theorem~\ref{th:lambda-centrality}, Lemma~\ref{lm:saddle-is-maximizer}

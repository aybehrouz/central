...
Thus, one natural way for defining a query set $\Xpairs$ is to let each \rv{x} be the outcome of
...
\begin{equation*}
    \KLcond[\Cx]{\vval{x}}{\vval{e}} = \lambda
\end{equation*}
\emph{derive a $\paramValue = (\paramA, \paramB)$ closed form for central dist...}

\[
    \frac{\pi(\vval{x} \mid \vval{e}, \paramValue)}{\Cx(\vval{x} \mid \vval{e})}
    = \frac{\pi(\paramValue \mid \vval{x}, \vval{e})}{\Cx(\paramValue \mid \vval{e})}
\]
